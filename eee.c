...
int init_module(void) {
	...
	int n,m;
	eee_get_freq(&n,&m);
	printk(KERN_NOTICE "Currently at: %d %d\n",n,m);
	eee_set_freq(100, 24);
    eee_set_voltage(1);
    printk(KERN_NOTICE "Asus eeePC extras, version %s.\n", EEE_VERSION);
	eee_get_freq(&n,&m);
	printk(KERN_NOTICE "POST-OC - Currently at: %d %d. Cpu temp: %d\n",n,m,eee_get_temperature());
    return 0;
}
...
void cleanup_module(void) {
	...
	eee_set_freq(70, 24);
    eee_set_voltage(1);
    printk(KERN_NOTICE "Asus eeePC extras takedown, version %s\n", EEE_VERSION);
    //eee_proc_cleanup();
}
